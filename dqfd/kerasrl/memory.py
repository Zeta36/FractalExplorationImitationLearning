...
import random
import numpy as np
from dqfd.kerasrl.util import SumSegmentTree, MinSegmentTree
...
class RingBuffer(object):
    ...
    def __getitem__(self, idx):
        ...
        if idx < 0 or idx >= self.length:
            raise KeyError("Tried fo fetch index %s with self.length %s" % (idx, self.length))
...
class Memory(object):
    ...
    def get_recent_state(self, current_observation):
        ...
        for offset in range(0, self.window_length - 1):
            ...
            current_terminal = (
                self.recent_terminals[current_idx - 1] if current_idx - 1 >= 0 else False
            )
...
class PartitionedRingBuffer(object):
    ...
    def __getitem__(self, idx):
        if idx < 0:
            raise KeyError(
                "Tried fo fetch index %s with "
                "self.length %s and "
                "permanent_idx %s" % (idx, self.length, self.permanent_idx)
            )
...
class PrioritizedMemory(Memory):
    def __init__(self, limit, alpha=0.4, start_beta=1.0, end_beta=1.0, steps_annealed=1, **kwargs):
    ...
    def append(
        self, observation, action, reward, terminal, training=True
    ):  # super() call adds to the deques that hold the most recent info, which is fed to the agent
        # on agent.forward()
        super(PrioritizedMemory, self).append(
            observation, action, reward, terminal, training=training
        )
        if training:
            self.observations.append(observation)
...
class PartitionedMemory(Memory):
    def __init__(
        ...
        **kwargs
        ...
        # unpack the expert transitions (assumes order recorded by the kerasrl.utils.record_demo_data() method)
    ...
    def append(self, observation, action, reward, terminal, training=True):
        # super() call adds to the deques that hold the most recent info, which is fed to the agent
        # on agent.forward()
        super(PartitionedMemory, self).append(
            observation, action, reward, terminal, training=training
        )
        if training:
            self.observations.append(observation)
    ...
    def sample_by_idxs(self, idxs, batch_size, beta=1.0, nstep=1, gamma=1):
        """
        ...
        for idx in idxs:
            ...
            while terminal0:
                ...
                idx = sample_batch_indexes(
                    self.window_length + 1, self.nb_entries - nstep, size=1
                )[0]
                terminal0 = self.terminals[idx - 2]
    ...
    def update_priorities(self, idxs, priorities):
        ...
        for i, idx in enumerate(idxs):
            ...
            if idx < self.permanent_idx:
                priority = (priorities[i] ** self.alpha) + 0.999
            else:
                priority = priorities[i] ** self.alpha
            self.sum_tree[idx] = priority
            self.min_tree[idx] = priority
