...
class TestLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        print("Testing for {} episodes ...".format(self.params["n_episodes"]))
...
class TrainEpisodeLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        if self.filepath is not None:
            if os.path.exists(self.filepath):
                ...
                f.close()
        print("Training for {} steps ...".format(self.params["n_training_steps"]))
    ...
    def on_episode_end(self, episode, logs):
        ...
        n_step_digits = str(int(np.ceil(np.log10(self.params["n_training_steps"]))) + 1)
        template = (
            "{step: "
            + n_step_digits
            + "d}/{n_training_steps}: episode: {episode}, duration: {duration:.3f}s, episode steps: "
            "{episode_steps}, steps per second: {sps:.0f}, "
        ...
        variables = {
            "step": self.step,
            "n_training_steps": self.params["n_training_steps"],
            "episode": episode + 1,
            "duration": duration,
