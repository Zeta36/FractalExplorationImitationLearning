import numpy as np
from tensorflow.python.keras import initializers, regularizers, activations, constraints
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.kernel_constraint = (
            constraints.get(kernel_constraint) if kernel_constraint is not None else None
        )
        self.bias_constraint = (
            constraints.get(bias_constraint) if kernel_constraint is not None else None
        )
        self.kernel_regularizer = (
            regularizers.get(kernel_regularizer) if kernel_constraint is not None else None
        )
        self.bias_regularizer = (
            regularizers.get(bias_regularizer) if kernel_constraint is not None else None
        )
...
class NAFLayer(Layer):
    ...
    def __init__(self, n_actions, mode="full", **kwargs):
        ...
        if self.mode == "full":
            ...
            if K.backend() == "theano":
                ...
                def fn(x, L_acc, LT_acc):
                    x_ = K.zeros((self.n_actions, self.n_actions))
                    x_ = T.set_subtensor(x_[np.tril_indices(self.n_actions)], x)
                    diag = K.exp(T.diag(x_)) + K.epsilon()
                    x_ = T.set_subtensor(x_[np.diag_indices(self.n_actions)], diag)
                    return x_, x_.T

                outputs_info = [
                    K.zeros((self.n_actions, self.n_actions)),
                    K.zeros((self.n_actions, self.n_actions)),
                ]
                ...
                nb_elems = (self.n_actions * self.n_actions + self.n_actions) // 2
                ...
                diag_indeces = [0]
                for row in range(1, self.n_actions):
                ...
                tril_mask = np.zeros((self.n_actions, self.n_actions), dtype="int32")
                tril_mask[np.tril_indices(self.n_actions)] = range(1, nb_elems + 1)
            ...
            if K.backend() == "theano":
                ...
                def fn(x, P_acc):
                    x_ = K.zeros((self.n_actions, self.n_actions))
                    x_ = T.set_subtensor(x_[np.diag_indices(self.n_actions)], x)
                    return x_
                ...
                diag_mask = np.zeros((self.n_actions, self.n_actions), dtype="int32")
                diag_mask[np.diag_indices(self.n_actions)] = range(1, self.n_actions + 1)
                ...
                P = tf.scan(fn, L_flat, initializer=K.zeros((self.n_actions, self.n_actions)))
            else:
                raise RuntimeError('Unknown Keras backend "{}".'.format(K.backend()))
    ...
    def compute_output_shape(self, input_shape):
        ...
        if self.mode == "full":
            expected_elements = (self.n_actions * self.n_actions + self.n_actions) // 2
        elif self.mode == "diag":
            expected_elements = self.n_actions
        else:
        ...
        if input_shape[0][1] != expected_elements:
            raise RuntimeError(
                "Input 0 (L) should have {} elements but has {}".format(
                    expected_elements, input_shape[0][1]
                )
            )
        if input_shape[1][1] != self.n_actions:
            raise RuntimeError(
                "Input 1 (mu) should have {} elements but has {}".format(
                    self.n_actions, input_shape[1][1]
                )
            )
        if input_shape[2][1] != self.n_actions:
            raise RuntimeError(
