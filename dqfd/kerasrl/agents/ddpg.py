...
from dqfd.kerasrl.core import Agent
from dqfd.kerasrl.random import OrnsteinUhlenbeckProcess
...
class DDPGAgent(Agent):
    ...
    def __init__(
        ...
        gamma=0.99,
        batch_size=32,
        n_steps_warmup_critic=1000,
        n_steps_warmup_actor=1000,
        train_interval=1,
        memory_interval=1,
        delta_range=None,
        delta_clip=np.inf,
        ...
        **kwargs
        ...
        if hasattr(actor.output, "__len__") and len(actor.output) > 1:
            raise ValueError(
                'Actor "{}" has more than one output. DDPG expects an actor that has a single output.'.format(
                    actor
                )
            )
        if hasattr(critic.output, "__len__") and len(critic.output) > 1:
            raise ValueError(
                'Critic "{}" has more than one output. DDPG expects a critic that has a single output.'.format(
                    critic
                )
            )
        if critic_action_input not in critic.input:
            raise ValueError(
                'Critic "{}" does not have designated action input "{}".'.format(
        ...
        self.n_steps_warmup_actor = n_steps_warmup_actor
        self.n_steps_warmup_critic = n_steps_warmup_critic
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        if K.backend() == "tensorflow":
            self.actor_train_fn = K.function(
                critic_inputs + [K.learning_phase()], [self.actor(critic_inputs)], updates=updates,
            )
        else:
            if self.uses_learning_phase:
                critic_inputs += [K.learning_phase()]
            self.actor_train_fn = K.function(
                critic_inputs, [self.actor(critic_inputs)], updates=updates
            )
    ...
    def backward(self, reward, terminal=False):
        ...
        can_train_either = (
            self.step > self.n_steps_warmup_critic or self.step > self.n_steps_warmup_actor
        )
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.n_steps_warmup_critic:
            ...
            if self.step > self.n_steps_warmup_actor:
                ...
                assert action_values.shape == (self.batch_size, self.n_actions)
