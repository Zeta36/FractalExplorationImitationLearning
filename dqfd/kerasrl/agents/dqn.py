...
from dqfd.kerasrl.memory import PrioritizedMemory
from dqfd.kerasrl.layers import NAFLayer
...
class AbstractDQNAgent(Agent):
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        assert q_values.shape == (len(state_batch), self.n_actions)
        return q_values
...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if model.output._keras_shape != (None, self.n_actions):
            raise ValueError(
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        trainable_model = Model(
            inputs=ins + [y_true, importance_weights, mask], outputs=[loss_out, y_pred]
        )
    ...
    def backward(self, reward, terminal):
        ...
        if not self.training:
            # We're done here. No need to update the experience memory since we only
            # use the working memory to obtain the state over the most recent observations.
            return metrics
        ...
        if self.step > self.n_steps_warmup and self.step % self.train_interval == 0:
            ...
            if not self.prioritized:
                importance_weights = [1.0 for _ in range(self.batch_size)]
            ...
            metrics = self.trainable_model.train_on_batch(
                ins + [targets, importance_weights, masks], [dummy_targets, targets]
            )
...
class NAFAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        a_in = Input(shape=(self.n_actions,), name="action_input")
        ...
        A_out = NAFLayer(self.n_actions, mode=self.covariance_mode)([L_out, mu_out, a_in])
